...
describe("AuthController", () => {
  ...
  const mockAuthService = {
    ...
    findSessionByEmailAndDeviceId: jest.fn(
      (data: { email: string; device_id: string }) => {
  ...
  describe("sign In", () => {
    ...
    it("should return true if password are compare", async () => {
      ...
      // Actq
      const result = await mockAuthService.comparePassword(
        password_1,
        password_2,
      );
    ...
    it("should throw ApiRequestException", async () => {
      ...
      await expect(controller.signIn(invalidUserId)).rejects.toThrow(
        ApiRequestException,
      );
    });
    ...
    it("should throw ApiException no-user", async () => {
      ...
      jest
        .spyOn(securityService, "refresh")
        .mockResolvedValueOnce({ ...tokens });
      jest.spyOn(I18nContext, "current").mockReturnValue(i18n);
      await expect(controller.signIn(invalidUserEmail)).rejects.toThrow(
        ApiException,
      );
    });
    ...
    it("should throw ApiException wrong-password", async () => {
      ...
      jest.spyOn(I18nContext, "current").mockReturnValue(i18n);
      await expect(controller.signIn(invalidUserPassword)).rejects.toThrow(
        ApiException,
      );
    });
  });
  ...
  describe("sign In for admin", () => {
    it("should throw ApiRequestException", async () => {
      ...
      jest.spyOn(I18nContext, "current").mockReturnValue(i18n);
      await expect(controller.adminSignin(invalidUserId)).rejects.toThrow(
        ApiRequestException,
      );
    });
    ...
    it("should throw ApiException no-admin", async () => {
      ...
      jest.spyOn(I18nContext, "current").mockReturnValue(i18n);
      expect(controller.adminSignin(invalidAdminEmail)).rejects.toThrow(
        ApiException,
      );
    });
    ...
    it("should throw ApiException wrong-password", async () => {
      ...
      jest.spyOn(service, "comparePassword").mockResolvedValueOnce(false);
      jest.spyOn(I18nContext, "current").mockReturnValue(i18n);
      await expect(controller.adminSignin(invalidUserPassword)).rejects.toThrow(
        ApiException,
      );
    });
  });
  ...
  describe("sign up for users", () => {
    it("should throw ApiRequestException", async () => {
      ...
      jest.spyOn(I18nContext, "current").mockReturnValue(i18n);
      await expect(controller.signUp(invalidUserEmail)).rejects.toThrow(
        ApiRequestException,
      );
    });
    ...
    it("should throw ApiException no-admin", async () => {
      ...
      jest.spyOn(I18nContext, "current").mockReturnValue(i18n);
      await expect(controller.signUp(invalidAdminEmail)).rejects.toThrow(
        ApiException,
      );
    });
  });
  ...
  describe("sign out", () => {
    it("should throw ApiRequestException", async () => {
      ...
      jest.spyOn(I18nContext, "current").mockReturnValue(i18n);
      await expect(controller.signOut(user, invalidForm)).rejects.toThrow(
        ApiRequestException,
      );
    });
  });
  ...
  describe("forgot-password", () => {
    it("should throw ApiRequestException", async () => {
      ...
      jest.spyOn(I18nContext, "current").mockReturnValue(i18n);
      await expect(controller.forgotPassword(invalidUserEmail)).rejects.toThrow(
        ApiRequestException,
      );
    });
    ...
    it("should throw ApiException no session", async () => {
      ...
      jest.spyOn(I18nContext, "current").mockReturnValue(i18n);
      await expect(controller.forgotPassword(invalidSession)).rejects.toThrow(
        ApiException,
      );
    });
  });
  ...
  describe("reset-password", () => {
    it("should throw ApiRequestException", async () => {
      ...
      jest.spyOn(I18nContext, "current").mockReturnValue(i18n);
      await expect(controller.resetPassword(invalidUserEmail)).rejects.toThrow(
        ApiRequestException,
      );
    });
    ...
    it("should throw ApiException no session", async () => {
      const invalidSession = {
        ...reset_password,
        reset_token: "ee7c493a-31d9-48c0-8838-1f466eab181a",
      };
      let i18n: any = {
        t: (key: string) => {
          return key;
      ...
      jest
        .spyOn(service, "findSessionByResetToken")
        .mockResolvedValueOnce(null);
      jest.spyOn(I18nContext, "current").mockReturnValue(i18n);
      await expect(controller.resetPassword(invalidSession)).rejects.toThrow(
        ApiException,
      );
    });
  });
  ...
  describe("refresh", () => {
    it("should throw ApiException no-user", async () => {
      ...
      jest.spyOn(service, "getUserByEmail").mockResolvedValueOnce(null);
      jest.spyOn(I18nContext, "current").mockReturnValue(i18n);
      await expect(controller.refresh(invalidUserEmail)).rejects.toThrow(
        ApiException,
      );
    });
  });
