...
import { JwtAuthGuard } from '@app/libs/security/guards/security.guard';
...
describe('AuthController', () => {
  ...
  describe('getOneUserById', () => {
    ...
    it('should throw ApiException', async () => {
      ...
      await expect(controller.getOneUserById(invalidUserId)).rejects.toThrow(ApiException);
    });
  ...
  describe('updateUser', () => {
    ...
    it('should throw ApiException with ErrorCodes.NoUser if userService returns null', async () => {
      ...
      jest.spyOn(I18nContext, 'current').mockReturnValue(i18n);
      // Act and Assert
      await expect(controller.updateUser(invalidUserId)).rejects.toThrow(ApiException);
    });
  })
})
